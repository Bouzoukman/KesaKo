...
public class Watcher {
	 ...
	 */
	public Watcher(){
		//création du process d'indexation
		logger.debug("création du process d'indexation");
		IndexFileProcess iF=new IndexFileProcess(Parameters.getIndexInterval(),Parameters.getNbFile());
		iF.startWorking();
		//création du process d'extraction des meta données
		logger.debug("création du process d'extraction des meta-données");
		MetaFileProcess mF=new MetaFileProcess(Parameters.getIndexInterval(),Parameters.getNbFile());
		mF.startWorking();
		//création du process de réindexation des fichiers en erreur
		//régulièrement on retente l'indexation car Tika (outils de lecture des documents) à des erreurs
		logger.debug("création du process de réindexation");
		IndexFileErrorProcess iFE=new IndexFileErrorProcess(Parameters.getIndexErrorInterval());
		iFE.startWorking();
	...
	public static void main(String[] args) {
		DOMConfigurator.configure("log4j_watcher.xml");
